...
struct CharacterDetailView: View {
    ...
    private var element: DrakonElement {
        DrakonElement.parse(owned.base.energyType)
    }
    ...
    private var heroSection: some View {
        VStack(spacing: 12) {
            ...
            HStack(spacing: 8) {
                detailPill("LV \(owned.level)", tint: DrakonBladePalette.gold)
                detailPill(
                    owned.base.rarity.rawValue.uppercased(),
                    tint: owned.base.rarity.color
                )
                detailPill(element.title, tint: DrakonBladePalette.blue)
            }
        }
        .padding(18)
    ...
    private var progressionSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            DrakonStarRow(stars: owned.stars, size: 18)
    ...
    private func detailPill(_ text: String, tint: Color) -> some View {
        Text(text)
            ...
            .clipShape(DrakonBladeShape(pointDepth: 10, slant: 7))
            .overlay(
                DrakonBladeShape(pointDepth: 10, slant: 7)
