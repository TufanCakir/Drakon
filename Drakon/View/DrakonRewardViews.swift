...
struct DrakonRewardChip: View {
    ...
    var body: some View {
        ...
        .overlay(
            DrakonCutRectangle(cut: 7)
                .stroke(DrakonBladePalette.gold.opacity(0.7), lineWidth: 1)
        )
        .clipShape(DrakonCutRectangle(cut: 7))
        .accessibilityLabel("\(title) \(value)")
    }
}

struct DrakonRewardLine: View {
    let title: String
    let value: Int
    let icon: String
...
struct DrakonStarRow: View {
    let stars: Int
    var size: CGFloat = 14

    var body: some View {
        VStack(alignment: .leading, spacing: max(2, size * 0.20)) {
            starLine(range: 1...7, activeColor: DrakonBladePalette.gold)

            if stars > 7 {
                starLine(range: 8...14, activeColor: DrakonBladePalette.blue)
            }
        }
        ...
        HStack(spacing: max(1, size * 0.16)) {
            ForEach(Array(range), id: \.self) { index in
                BladeStar()
                    .fill(
                        index <= stars
                            ? activeColor : DrakonBladePalette.panelLight
                    )
